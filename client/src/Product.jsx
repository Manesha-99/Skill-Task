...
const Product = () => {
  ...
  return (
    <div className="flex flex-col justify-center bg-slate-300">
      <div className="bg-slate-600 p-4 rounded max-w-[600px] w-full mx-auto m-4">
      ...
      <div className="bg-slate-600 p-4 rounded max-w-[600px] w-full mx-auto m-4 ">
        <input
          ...
          className="rounded p-4 max-w-[250px] w-full text-center "
          value={searchItem}
          onChange={handleSearchChange}
        />
        <select
          className="rounded max-w-[250px] w-full p-4 ml-[60px] text-center cursor-pointer"
      ...
      <div className="">
        {filteredProducts.map((product) => (
          <div
            className="bg-slate-500 p-4 rounded max-w-[600px] w-full mx-auto m-4 text-center text-white"
            key={product.id}
          >
            <h2 className="text-[30px]">{product.name}</h2>
            <p className="text-[20px]">Category: {product.category}</p>
            <p className="text-[15px]">Price: ${product.price}</p>
          </div>
        ))}
      </div>
    </div>
